...
* 线层dir字段与拓扑方向
    线层的箭头方向为拓扑方向(即link层geometry中折点的行进方向)，dir字段所描述的行车方向就是与之关联的，dir为1代表该条link是单向路段，行车方向与拓扑方向一致，dir为0代表该条link是双向路段
...
|    o_x     | `float`  |         起点经度，不允许为空，GCJ-02坐标         |
|    o_y     | `float`  |        起点纬度，不允许为空，GCJ-02坐标系         |
|    d_x     | `floatg` |        终点经度，不允许为空，GCJ-02坐标系         |
...
|:-----------:|:-----------:|:-----------:|:---:|:-------------:|:---:|-----------:|
| **平面投影CRS** | EPSG:32601  | EPSG:32602  | ... |  EPSG:32649   | ... | EPSG:32660 |
